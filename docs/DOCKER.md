...
### Vì sao chọn phiên bản này
| Thành phần | Chọn | Lý do |
|---|---|---|
| MySQL | `mysql:9.7` | Đây là nhánh **LTS mới nhất** (Oracle hỗ trợ tới 21/04/2034). MySQL 8.0 đã EOL từ 21/04/2026. **Không dùng `mysql:latest`** vì tag đó trỏ nhánh Innovation (chỉ hỗ trợ 6 tháng). |
| Object storage | `rustfs/rustfs:latest` | RustFS là bản viết lại MinIO bằng Rust, giấy phép **Apache 2.0** cho phép dùng thương mại. MinIO dùng AGPL và đã **archive bản community ngày 25/04/2026**, gỡ sạch khỏi Docker Hub (quay.io trả 401). Cùng port 9000/9001 và cùng bộ lệnh S3 nên code `@aws-sdk/client-s3` không phải sửa. |
| Caddy | `caddy:latest` | Nhánh 2.x ổn định. |
...
> Muốn dùng AWS S3/R2 thay RustFS: sửa các biến `S3_*` của backend là xong.
...
## 5. Vận hành
```bash
docker compose --env-file .env.docker logs -f [mysql|backend|objectstore|admin|caddy]
...
# Backup ảnh
docker run --rm -v unimate_objectstore-data:/data -v /backup:/b alpine tar czf /b/objectstore-$(date +%F).tar.gz /data
...
- File công khai: qua `/files` (Caddy ngoài hoặc profile edge) trỏ tới RustFS `:9000` · Console RustFS: `http://127.0.0.1:9001` trên máy chủ.
- Test không cần domain: `API_DOMAIN=http://IP` + thêm `FORCE_HTTPS=0`, `COOKIE_SECURE=0`.

## 6. Sự cố thường gặp| Hiện tượng | Nguyên nhân / cách sửa |
|---|---|
| Backend `Restarting`, log `ECONNREFUSED mysql:3306` | MySQL chưa mở TCP xong — backend tự thử lại ~60s rồi mới chạy; cứ đợi |
| `objectstore` restart liên tục, log `permission denied` khi tạo thư mục dữ liệu | Image RustFS chạy non-root (UID 10001) còn volume do Docker tạo thuộc root. Compose đã đặt `user: root` — đừng xoá dòng đó khi chạy stack local |
| Muốn đổi sang RustFS từ bản cũ dùng MinIO | Xoá volume cũ `docker volume rm unimate_minio-data` (không migrate được, format khác nhau) rồi `docker exec unimate-backend-1 npm run db:seed-demo` để nạp lại ảnh |
